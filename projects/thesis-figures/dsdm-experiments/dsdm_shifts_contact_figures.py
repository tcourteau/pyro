...
class plotter(object):
    ...
    def flip_axis( self ):
        ...
        self.w1  = self.w1  * -1.0
        self.w2  = self.w2  * -1.0
        self.wr1 = self.wr1 * -1.0
        self.wr2 = self.wr2 * -1.0
        
        
    def filter_m1( self, cut = 3 ):
        
        # Filtering
        low_pass_filter      = filters.low_pass( fc = cut , dt = 0.002 ) 
        self.dq = low_pass_filter.filter_array( self.dq  )
        self.w1 = low_pass_filter.filter_array( self.w1  )
        self.w2 = low_pass_filter.filter_array( self.w2  )
        
        self.wr1 = low_pass_filter.filter_array( self.wr1  )
        self.wr2 = low_pass_filter.filter_array( self.wr2  )
        
        
    def average_m1( self, n = 20.0 ):
        
        # Running Average
        self.dq = np.convolve( self.dq , np.ones((n,))/n, mode='same')
        self.w1 = np.convolve( self.w1 , np.ones((n,))/n, mode='same')
        self.w2 = np.convolve( self.w2 , np.ones((n,))/n, mode='same')
        
        self.wr1 = np.convolve( self.wr1 , np.ones((n,))/n, mode='same')
        self.wr2 = np.convolve( self.wr2 , np.ones((n,))/n, mode='same')
    ...
    def plot_contact(self , filename ):
        ...
        plot[0].plot( self.t ,  self.q , 'b' ,  label = 'Actual')
        #plot[0].set_yticks([0,100])
        plot[0].set_ylim( -0.5 , round( self.q.max() ) +0.5)
        plot[0].grid(True)
        #legend = plot[0].legend(loc='lower right', fancybox=True, shadow=False, prop={'size':fontsize})
        #legend.get_frame().set_alpha(0.4)
        ...
        plot[2].grid(True)
        plot[2].set_yticks([0,250,500])
        plot[2].set_ylim(-100,500)
        legend = plot[2].legend(loc='lower right', fancybox=True, shadow=False, prop={'size':fontsize})
        legend.get_frame().set_alpha(0.4)
...
#
#pltr.loaddata( name , i , j )
#pltr.plot_contact( 'dsdm_stiff_down_latency_zoom' )
